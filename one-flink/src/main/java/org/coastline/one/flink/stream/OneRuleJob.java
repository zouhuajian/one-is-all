...
import org.coastline.one.flink.stream.filter.DataFilterFunction;
...
import org.coastline.one.flink.stream.source.NumberSource;
...
public class OneRuleJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // add source
        SingleOutputStreamOperator<JSONObject> singleOutputStream = env.addSource(new NumberSource())
                // source 设置并行为1
                ...
                }).setParallelism(2).name("side_out_for_hbase");
        ...
        SingleOutputStreamOperator<AggregateData> outputStream = singleOutputStream
                ...
                // 过滤没有超过阈值的数据
                .filter(new CompareFilterFunction()).setParallelism(2).name("filter_good");
